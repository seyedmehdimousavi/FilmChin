...
function findSearchSuggestionPrefix(query, list) {
  ...
  const titleMatch = (list || []).find((item) => {
    const title = String(item?.title || item?.name || "").trim().toLowerCase();
    return title.startsWith(lowerRaw);
  });

  if (!titleMatch) return "";
  const label = String(titleMatch.title || titleMatch.name || "").trim();
  return label.length > rawQuery.length ? label.slice(rawQuery.length) : "";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (searchInput) {
    ...
    searchGhostHint?.addEventListener("click", () => {
